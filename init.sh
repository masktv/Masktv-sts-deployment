...
if [[ "$MYSQL_REPLICATION_MODE" == "slave" ]]; then
  ...
  # Wait for the master to be fully ready
  while ! mysql -u root -p"$MYSQL_ROOT_PASSWORD" -h "$MYSQL_MASTER_HOST" -e "SHOW MASTER STATUS;" > /dev/null 2>&1; do
    echo "Waiting for master to be ready..."
    sleep 5
  done

  # Fetch the master log file and position from the master
  MASTER_STATUS=$(mysql -u root -p"$MYSQL_ROOT_PASSWORD" -h "$MYSQL_MASTER_HOST" -e "SHOW MASTER STATUS;" | tail -n 1)
